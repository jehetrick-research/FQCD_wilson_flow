...
int main(int argc, char** argv) {
  ...
  gauge_field	W1(lattice,nc);   // declare SU(3) field
  gauge_field	W2(lattice,nc);   // declare SU(3) field
  ...
  for(i=1; i<=trajecs; i++) {
	...
	if(ME==0) { cout<<"Doing Wilson Flow for first time U->W1"<<endl;
	   wilsonFlow(U, W1, S, 0.01, 5);
	   plaq = average_plaquette(W1);
	}
	if(ME==0) {
	   cout <<"WF_MEAS "<< beta <<" "<< plaq <<endl; 
	}
	// Flip-flop WF between W1 and W2
	for(int nwf=0; nwf<50; nwf++) {
	   wilsonFlow(W1, W2, S, 0.01, 5);
	   plaq = average_plaquette(W2);
	   if(ME==0) {
	      cout <<"WF_MEAS W1->W2 "<< beta <<" "<< plaq <<" "<< nwf <<endl; 
	   }
	   wilsonFlow(W2, W1, S, 0.01, 5);
	   plaq = average_plaquette(W1);
	   if(ME==0) {
	      cout <<"WF_MEAS W2->W1 "<< beta <<" "<< plaq <<" "<< nwf <<endl; 
	   }
	}  
     }
